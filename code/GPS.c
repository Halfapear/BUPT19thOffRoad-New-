...
void GPS_Record_flash() { // 将采集的点位记录到缓冲区并储存至GPS_FLASH
    ...
    printf("\r\n缓冲区纬度数据:%d %d", Number,(int)flash_union_buffer[Number].uint32_type);
    ...
    printf("\r\n缓冲区经度数据:%d %d", Number,(int)flash_union_buffer[Number].uint32_type);
    ...
    printf("\r\n缓冲区11经度数据:%d %d", Number,(int)flash_union_buffer[1].uint32_type);
...
void GPS_Work_array() { // 用于隔离FLASH,实际参加计算的数组
    for (int NUM = 0; NUM < (ARRAY_Element_Calculation() / 2); NUM++) {
        ...
        printf("\r\n组数-%d,工作数组纬度数据-%f,工作数组经度数据-%f", NUM + 1, Target_point[0][NUM], Target_point[1][NUM]);
        system_delay_ms(50);
    }
}
...
void GPS_Record_flash() { // 将采集的点位记录到缓冲区并储存至GPS_FLASH
    ...
    //flash_union_buffer[Number].float_type = Number+1; // 将纬度数据强制转换后储存在FLASH 操作的数据缓冲区 (数组标号为偶数时是纬度数据)
    ips200_show_string(0, 16 * 0, "R:");
...
double lat; // 纬度
double lot; // 经度
double Start_Point[2];//记录开始发车的角度
