...
#define Float_Record_FLAG  (1)          //单精度浮点存储开启标志位//可用
#define Double_Record_FLAG (0)          //双精度浮点存储开启标志位//可用
#define Int_Record_FLAG    (0)          //整数存储开启标志位     //可用
#define Array_Record_FLAG  (0)          //数组存储标志位        //试用
...
#define MIC_Start_FLAG        (1)        //开启硅麦--硅麦采集是不关的，这个标志是复制出来计算硅麦时用的

#define XF6_Control_Flag      (1)       //遥控停车标志位，0的时候遥控失效

#define MIC_Mode           (0)           //这三个标志位都是为FLOOWTRACK函数准备的
#define GPS_Mode           (0)
#define GPS_MIC_Mode       (1)
