...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
double get_two_points_distance (uint32 latitude1, uint32 longitude1, uint32 latitude2, uint32 longitude2)//将形参改为uint32类型
{
    const double EARTH_RADIUS = 6378137;                                        // 地球半径(单位：m)
    double rad_latitude1;
    double rad_latitude2;
    ...
    rad_latitude1 = ANGLE_TO_RAD(latitude1)  /1000000.0f;                                    // 根据角度计算弧度
    rad_latitude2 = ANGLE_TO_RAD(latitude2)  /1000000.0f;
    rad_longitude1 = ANGLE_TO_RAD(longitude1)/1000000.0f;
    rad_longitude2 = ANGLE_TO_RAD(longitude2)/1000000.0f;
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
double  get_two_points_azimuth (uint32 latitude1, uint32 longitude1, uint32 latitude2, uint32 longitude2)//将形参改为uint32类型
{

    double latitude1_temp =  ANGLE_TO_RAD(latitude1)/100.0f ;
    double latitude2_temp =  ANGLE_TO_RAD(latitude2)/100.0f ;
    double longitude1_temp = ANGLE_TO_RAD(longitude1)/100.0f;
    double longitude2_temp = ANGLE_TO_RAD(longitude2)/100.0f;

    double x =  sin(longitude2_temp - longitude1_temp) * cos(latitude2_temp);
    double y =  cos(latitude1_temp) * sin(latitude2_temp) - sin(latitude1_temp) * cos(latitude2_temp) * cos(longitude2_temp - longitude1_temp);
    double angle = RAD_TO_ANGLE(atan2f(x, y));
...
//    double distance, distance1, distance10, distance11, distance12, distance13, distance14, distance2, distance3;
    ...
    distance = 2 * py_asin(func_sqrt(func_pow(func_sin(a / 2), 2) + func_cos(rad_latitude1) * func_cos(rad_latitude2) * func_pow(func_sin(b / 2), 2)));   // google maps 里面实现的算法
    distance = distance * EARTH_RADIUS;
...
#if USART_Send_STOP_FLAG
                    uart_write_buffer(UART_2, &gps_rmc_buffer[GPS_TAU1201_BUFFER_SIZE],temp_length);
#endif
                }
            }
...
#if USART_Send_STOP_FLAG
                    uart_write_buffer(UART_2, &gps_gga_buffer[GPS_TAU1201_BUFFER_SIZE],temp_length);
#endif
                }
            }
